...
- `auto_queue_runs` (default `never` so create does not fan out 15 runs)
- `aws_provider_configuration_id` (`pcfg-...`) for `provider_aws_*` only
...
4. Apply → 15 workspaces under current env, each with working_directory `config/reports/semver_sort/<fixture>`.
